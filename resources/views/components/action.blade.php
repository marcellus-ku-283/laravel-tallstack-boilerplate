@props(['model', 'route' => null, 'deleteClick'])

@if (!empty($route) && !empty($model))
    <a href="{{ route($route . '.show', $model->id) }}"
        ...
        x-data="{ tooltip: 'View {{ $route }}' }" x-tooltip="tooltip">
    ...
    <a href="{{ route($route . '.edit', $model->id) }}"
        class="inline-flex text-sm font-semibold tracking-widest uppercase transition duration-150 ease-in-out border border-transparent rounded-md items-centerpy-2 text-primary"
        x-data="{ tooltip: 'Edit {{ $route }}' }" x-tooltip="tooltip">
    ...
    @if (!empty($deleteClick))
        <button type="button" wire:click="{{ $deleteClick.'('.$model->id.')' }}"
            class="inline-flex text-sm font-semibold tracking-widest uppercase transition duration-150 ease-in-out border border-transparent rounded-md items-centerpy-2 text-primary"
            x-data="{ tooltip: 'Delete {{ $route }}' }" x-tooltip="tooltip">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round"
